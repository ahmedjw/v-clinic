  ...
  private async createMockPatientRecord(user: User): Promise<void> {
    try {
      ...
      const mockPatientData = {
        name: user.name,
        email: user.email,
        phone: user.email === "patient@clinic.com" ? "+1-555-0123" : "+1-555-0124",
        dateOfBirth: user.email === "patient@clinic.com" ? "1990-05-15" : "1985-08-22",
        gender: user.email === "patient@clinic.com" ? ("male" as const) : ("female" as const),
        address:
          user.email === "patient@clinic.com" ? "123 Main St, City, State 12345" : "456 Oak Ave, City, State 12345",
        emergencyContact: {
          name: user.email === "patient@clinic.com" ? "Mary Doe" : "Bob Smith",
          phone: user.email === "patient@clinic.com" ? "+1-555-0199" : "+1-555-0188",
          relationship: user.email === "patient@clinic.com" ? "Spouse" : "Brother",
        },
        medicalHistory:
          user.email === "patient@clinic.com"
            ? "No significant medical history"
            : "Hypertension, managed with medication",
        allergies: user.email === "patient@clinic.com" ? ["Penicillin"] : ["Shellfish", "Pollen"],
        currentMedications: user.email === "patient@clinic.com" ? [] : ["Lisinopril 10mg"],
      ...
      const patient = await localDB.getPatients().then((patients) => patients.find((p) => p.email === user.email))
      ...
      if (patient) {
        ...
        await localDB.addMedicalRecord({
          patientId: patient.id,
          doctorId: "doctor-1",
          date: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0], // Last month
          diagnosis: user.email === "patient@clinic.com" ? "Annual Physical Exam" : "Hypertension Follow-up",
          symptoms: user.email === "patient@clinic.com" ? ["None"] : ["Mild headaches"],
          treatment: user.email === "patient@clinic.com" ? "Continue healthy lifestyle" : "Continue current medication",
          prescription:
            user.email === "patient@clinic.com"
              ? []
              : [
          ...
          vitals: {
            bloodPressure: user.email === "patient@clinic.com" ? "120/80" : "135/85",
            heartRate: user.email === "patient@clinic.com" ? 72 : 78,
            temperature: 36.5,
            weight: user.email === "patient@clinic.com" ? 70 : 60,
            height: user.email === "patient@clinic.com" ? 175 : 165,
          },
          notes: user.email === "patient@clinic.com" ? "Patient in good health" : "Blood pressure well controlled",
        })
      }
    } catch (error) {
