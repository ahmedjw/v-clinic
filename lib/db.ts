...
export interface Patient extends User {
  ...
  assignedDoctorIds?: string[]; // Array of doctor IDs assigned to the patient
  assignedDoctorNames?: string[]; // Array of doctor names for easier display
  assignedDoctorEmails?: string[]; // Array of doctor emails for easier display
  assignedDoctorPhones?: string[]; // Array of doctor phones for easier display
  dateOfBirth?: string; // Date of Birth (YYYY-MM-DD)
  bloodType?: string; // Blood type (A+, O-, etc.)
  allergies: Array<string>; // Allergies information
  currentMedications?: Array<string>; // Current medications
  emergencyContact: {
    name: string;
    relationship: string;
    phone: string;
...
export interface Appointment extends BaseData {
  ...
  date: string | number | Date; // YYYY-MM-DD
  time: string; // HH:MM
  reason: string;
...
export class LocalDB {
  ...
  private async add<T extends BaseData>(
    ...
    await db.add(storeName as any, record as any); // Cast to any to satisfy idb's put type
    return record;
  }
  ...
  private async update<T extends BaseData>(
    ...
    await db.put(storeName as any, updatedRecord as any); // Cast to any
  ...
  private async getAll<T extends BaseData>(
    ...
    const db = await this.getDb();
    return (await db.getAll(storeName as any)) as any[];
  }
